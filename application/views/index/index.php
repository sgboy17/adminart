...
$total = $this->db->query('SELECT s.* FROM '.$this->prefix.'student s WHERE s.deleted = 0')->num_rows();
							...
							<script type="text/javascript">
								...
								function drawChart() {
									...
									var data = google.visualization.arrayToDataTable([
										['Ngày', 'Số học viên'],
										<?php $result_report_day = $report_day->result(); ?>
										<?php if($report_day->num_rows() >= 365) $limit = $report_day->num_rows()-365; else $limit = 0; ?>
										<?php for($i=$report_day->num_rows()-1-$limit; $i>=0; $i--){ ?>
										<?php $row = $result_report_day[$i]; ?>
										['<?php echo format_get_date($row->profit_date,'d/m') ?>', <?php echo !empty($row->profit)?$row->profit:'0' ?>],
										<?php } ?>
									]);
						...
						<div class="col-sm-12 col-md-12">
							<script type="text/javascript">
								...
								function drawChart1() {
									...
									var data = google.visualization.arrayToDataTable([
										['Nhóm', 'Số học viên'],
										['Tiềm năng', <?php echo $total_0 ?>],
										['Mới', <?php echo $total_1 ?>],
										['Còn lại', <?php echo $total_2 ?>],
									]);
							...
							<script type="text/javascript">
								...
								function drawChart2() {
									var data = google.visualization.arrayToDataTable([
										['Ngày', 'Số học viên'],
										<?php $result_report_day = $report_day->result(); ?>
										<?php if($report_day->num_rows() >= 7) $limit = $report_day->num_rows()-7; else $limit = 0; ?>
										<?php for($i=$report_day->num_rows()-1-$limit; $i>=0; $i--){ ?>
										<?php $row = $result_report_day[$i]; ?>
										['<?php echo format_get_date($row->profit_date,'d/m') ?>', <?php echo !empty($row->profit)?$row->profit:'0' ?>],
										<?php } ?>

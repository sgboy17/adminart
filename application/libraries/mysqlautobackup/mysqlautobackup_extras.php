...
{
    $fp = fopen(FCPATH . "/backups/" . $backup_file_name, "w");
    fwrite($fp, $gzdata);
    fclose($fp);
    //check folder is protected - stop HTTP access
    if (!file_exists(FCPATH . "/backups/.htaccess")) {
        $fp = fopen(FCPATH . "/backups/.htaccess", "w");
...
function output_backup($gzdata, $backup_file_name)
{
    ...
    public function transfer_data($ftp_username, $ftp_password, $ftp_server, $ftp_path, $filename)
    {
        if (function_exists('curl_exec')) {
            ...
            if (empty($info['http_code'])) $this->error = NEWLINE . "FTP ERROR - Failed to transfer backup file to remote ftp server";
                ...
                $http_codes = parse_ini_file(APPPATH . "libraries/mysqlautobackup/http_codes.ini");
...
class dbc extends PDO
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            $this->reportDBError($e->getMessage());
        }
    }
    ...
    {
        try {
            return $this->dbconn->bindParam($query);
        } catch (PDOException $e) {
            $this->reportDBError($e->getMessage());
        }
    }
    ...
    public function query($query)
    {
        try {
            ...
            $this->reportDBError($e->getMessage() . "<hr>" . $e->getTraceAsString());
        }
    }

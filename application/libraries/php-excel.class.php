...
class Excel_XML
{
     ...
     */
         ...
         */
        private function addRowFirst ($array)
        {
            $cells = "";
                foreach ($array as $k => $v):
                        ...
                        $v = htmlspecialchars($v, ENT_QUOTES, $this->sEncoding);
                        $cells .= "<Cell ss:StyleID=\"s1\"><Data ss:Type=\"$type\">" . str_replace('&amp;#10;', '&#10;', $v) . "</Data></Cell>\n"; 
                endforeach;
        ...
        private function addRow ($array)
        {
            $cells = "";
                foreach ($array as $k => $v):
                        ...
                        $v = htmlspecialchars($v, ENT_QUOTES, $this->sEncoding);
                        $cells .= "<Cell ss:StyleID=\"s2\"><Data ss:Type=\"$type\">" . str_replace('&amp;#10;', '&#10;', $v) . "</Data></Cell>\n"; 
                endforeach;
                $this->lines[] = "<Row ss:AutoFitHeight=\"0\">\n" . $cells . "</Row>\n";
        }
         ...
         */
        public function addArray ($array, $size)
        {   
            foreach($size as $row){
                $this->lines[] = "<Column ss:AutoFitWidth=\"0\" ss:Width=\"".$row."\"/>\n";
            }
                foreach ($array as $k => $v){
                    if($k==0) $this->addRowFirst ($v, $size);
                    else $this->addRow ($v, $size);
                }
        }

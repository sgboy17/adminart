if($('#branch_root_list').length){
	var branch_list = $('#branch_root_list').dataTable({
		    "bProcessing": true,
		    "bServerSide": true,
		    "sPaginationType": "full_numbers",
		    "sAjaxSource": base_url+"branchroot/get_branch_list",
		    "bDeferRender": true,
		    "bLengthChange": false,
		    "bFilter": false,
		    "bDestroy": true,
...
function delete_branch_multiple(){
    if(confirm('Bạn có chắc muốn xoá những dữ liệu này?')){
        ...
        jQuery.ajax({
            type: 'POST',
            url: base_url+'branchroot/delete_branch',
            data: {
                id: id
            },
...
function delete_branch(id){
	if(confirm('Bạn có chắc muốn xoá dữ liệu này?')){
	    jQuery.ajax({
	        ...
	        success:function(resp){
               if(resp=='BRANCH') document.location.href = document.location.href;
	           if($('.paginate_active').length) $('.paginate_active').trigger('click');
	           else document.location.href = document.location.href ;
	           $.unblockUI();
	        }
...
function load_branch_add(){
    jQuery.ajax({
        type: 'POST',
        url: base_url+'branchroot/load_branch_add',
        beforeSend: function(xhr) {
...
function load_branch_edit(id){
    jQuery.ajax({
        type: 'POST',
        url: base_url+'branchroot/load_branch_edit',
        data: {
            id: id,
        },
        beforeSend: function(xhr) {
...
function save_branch_add(element){
    if($("#branch_form").valid()){
         jQuery.ajax({
           type:'POST',
           url:base_url+'branchroot/save_branch_add',
...
function save_branch_edit(element){
    if($("#branch_form").valid()){
         jQuery.ajax({
           type:'POST',
           url:base_url+'branchroot/save_branch_edit',
           data:$('#branch_form').serialize(),
           beforeSend: function(xhr) {
                $.blockUI({css: {
                    border: 'none',

if($('#object_list').length){
	var object_list = $('#object_list').dataTable({
		    "bProcessing": true,
		    "bServerSide": true,
		    "sPaginationType": "full_numbers",
		    "sAjaxSource": base_url+"object/get_object_list",
		    "bDeferRender": true,
		    "bLengthChange": false,
		    "bFilter": false,
		    "bDestroy": true,
...
function delete_object_multiple(){
    if(confirm('Bạn có chắc muốn xoá những dữ liệu này?')){
        ...
        jQuery.ajax({
            type: 'POST',
            url: base_url+'object/delete_object',
            data: {
                id: id
            },
...
function validate_object_form(){
    $("#object_form").validate({
        ...
        messages: {
            ...
            type: "Vui lòng điền loại nhóm đối tượng!",
...
function load_object_add(){
    jQuery.ajax({
        type: 'POST',
        url: base_url+'object/load_object_add',
        beforeSend: function(xhr) {
            jQuery("#object_detail #object_form").html('<div align="center" style="margin: 10px;"><img src="'+base_url+'resources_admin/images/loaders/circular/072.gif" /></div>');      
        },
        error:function(){
        ...
        success:function(resp){
            jQuery("#object_detail #object_form").html(resp);
            validate_object_form();
        }
    });  
}

function load_object_edit(id){
    jQuery.ajax({
        type: 'POST',
        url: base_url+'object/load_object_edit',
        data: {
            id: id,
        },
        beforeSend: function(xhr) {
...
function save_object_add(element){
    if($("#object_form").valid()){
         jQuery.ajax({
           type:'POST',
           url:base_url+'object/save_object_add',
...
function save_object_edit(element){
    if($("#object_form").valid()){
         jQuery.ajax({
           type:'POST',
           url:base_url+'object/save_object_edit',
           data:$('#object_form').serialize(),
           beforeSend: function(xhr) {
                $.blockUI({css: {
                    border: 'none',

if($('#store_export_group_list').length){
	var store_export_group_list = $('#store_export_group_list').dataTable({
		    "bProcessing": true,
		    "bServerSide": true,
		    "sPaginationType": "full_numbers",
		    "sAjaxSource": base_url+"storeexportgroup/get_store_export_group_list",
		    "bDeferRender": true,
		    "bLengthChange": false,
		    "bFilter": false,
		    "bDestroy": true,
     ...
     $("#search").on("keyup", function () {
        store_export_group_list.fnDraw(); 
    });
...
function store_export_group_list_filter(){
    $('#type_search').val('1');
     store_export_group_list.fnDraw(); 
     $('.modal .close').trigger('click');
}

function delete_store_export_group_multiple(){
    if(confirm('Bạn có chắc muốn xoá những dữ liệu này?')){
        ...
        jQuery.ajax({
            type: 'POST',
            url: base_url+'storeexportgroup/delete_store_export_group',
            data: {
                id: id
            },
...
function delete_store_export_group(id){
...
function validate_store_export_group_form(){
    $("#store_export_group_form").validate({
        ignore: null,
        ...
        messages: {
            ...
            created_date: "Vui lòng điền ngày xuất!",
    ...
    if($('#store_export_list').length){
        var store_export_list = $('#store_export_list').dataTable({
                "bProcessing": true,
                "bServerSide": true,
                "sPaginationType": "full_numbers",
                "sAjaxSource": base_url+"storeexport/get_store_export_list",
                "bDeferRender": true,
                "bLengthChange": false,
                "bFilter": false,
                "bDestroy": true,
                ...
                if($('#id').length) aoData.push( { "name": "f_store_export_group_id", "value": $('#id').val() } );
...
function load_store_export_group_add(){
    jQuery.ajax({
        type: 'POST',
        url: base_url+'storeexportgroup/load_store_export_group_add',
        beforeSend: function(xhr) {
            jQuery("#store_export_group_detail #store_export_group_form").html('<div align="center" style="margin: 10px;"><img src="'+base_url+'resources_admin/images/loaders/circular/072.gif" /></div>');      
        },
        error:function(){
        ...
        success:function(resp){
            jQuery("#store_export_group_detail #store_export_group_form").html(resp);
            validate_store_export_group_form();
        }
    });  
}

function load_store_export_group_edit(id){
    jQuery.ajax({
        type: 'POST',
        url: base_url+'storeexportgroup/load_store_export_group_edit',
        data: {
            id: id,
        },
        beforeSend: function(xhr) {
...
function save_store_export_group_add(element){
    var quantity = '';
    var key = '';
    ...
    if($("#store_export_group_form").valid()){
         jQuery.ajax({
           type:'POST',
           url:base_url+'storeexportgroup/save_store_export_group_add',
           data:$('#store_export_group_form').serialize(),
           beforeSend: function(xhr) {
                $.blockUI({css: {
                    border: 'none',
                    padding: '15px',
            ...
            success:function(resp){
                ...
                jQuery("#store_export_group_detail #store_export_group_form").html('');
                if($('.paginate_active').length) $('.paginate_active').trigger('click');
                else document.location.href = document.location.href ;
                $.unblockUI();
            }
...
function save_store_export_group_edit(element){
    var quantity = '';
    var key = '';
    var store_export_id = '';
    var id_textbox = $('form .quantity');
    ...
    if($("#store_export_group_form").valid()){
         jQuery.ajax({
           type:'POST',
           url:base_url+'storeexportgroup/save_store_export_group_edit',
           data:$('#store_export_group_form').serialize(),
           beforeSend: function(xhr) {
                $.blockUI({css: {
                    border: 'none',

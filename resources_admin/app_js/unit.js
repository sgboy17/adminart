if($('#unit_list').length){
	var unit_list = $('#unit_list').dataTable({
		    "bProcessing": true,
		    "bServerSide": true,
		    "sPaginationType": "full_numbers",
		    "sAjaxSource": base_url+"unit/get_unit_list",
		    "bDeferRender": true,
		    "bLengthChange": false,
		    "bFilter": false,
		    "bDestroy": true,
...
function load_unit_add(){
    jQuery.ajax({
        type: 'POST',
        url: base_url+'unit/load_unit_add',
        beforeSend: function(xhr) {
            jQuery("#unit_detail #unit_form").html('<div align="center" style="margin: 10px;"><img src="'+base_url+'resources_admin/images/loaders/circular/072.gif" /></div>');      
        },
        error:function(){
        ...
        success:function(resp){
            jQuery("#unit_detail #unit_form").html(resp);
            validate_unit_form();
        }
    });  
}

function load_unit_edit(id){
    jQuery.ajax({
        type: 'POST',
        url: base_url+'unit/load_unit_edit',
        data: {
            id: id,
        },
        beforeSend: function(xhr) {
...
function save_unit_add(element){
    if($("#unit_form").valid()){
         jQuery.ajax({
           type:'POST',
           url:base_url+'unit/save_unit_add',
...
function save_unit_edit(element){
    if($("#unit_form").valid()){
         jQuery.ajax({
           type:'POST',
           url:base_url+'unit/save_unit_edit',
           data:$('#unit_form').serialize(),
           beforeSend: function(xhr) {
                $.blockUI({css: {
                    border: 'none',
